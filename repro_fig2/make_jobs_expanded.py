...
Default preset:
  5 datasets x 3 models x 3 layer sizes x 6 sample sizes x 3 seeds = 810 jobs

Use --one_seed for a cheaper first pass:
  5 x 3 x 3 x 6 x 1 = 270 jobs
"""
        ...
        for model in MODELS:
            for layers, heads in LAYERS:
                for sample in SAMPLES:
                    for seed in seeds:
                        rows.append(f"{model}\t{dataset}\t{sample}\t{seed}\t{layers}\t{heads}\n")
