...
以同时存在 `args.json`、`metrics.json` 和 `predictions.csv` 作为一次实验完成的判据：
...
2048 样本下已经完成的 8 个结果全部属于 AG News：

- ENC：1、6、12 层
- AR：1、6、12 层
- MLM：1、6 层

MLM 12 层在约 75% 训练进度处被中断。因此论文统一排除全部 2048 结果，避免只展示单个数据集和不完整模型网格造成选择偏差。

## 2. Figure 2 部分复现的主要结果
...
当前单峰率明显低于原论文在大样本区间报告的水平，说明 1024 个样本不足以让随机初始化的 12 层模型稳定学习五级情感顺序。
...
- 完整样本范围只到 1024，尚未进入论文最重要的高数据区间。
- 未包含 DIFF 和 AR-pseudo。
- 全部模型从零初始化，不代表预训练模型微调结果。
- 当前结果适合形成课程论文的阶段性分析，但最终论文应在补齐更多样本量或随机种子后重新生成统计图表。
